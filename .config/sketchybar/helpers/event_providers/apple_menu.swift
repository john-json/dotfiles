...
typealias MRMediaRemoteGetNowPlayingApplicationDisplayNameFunction = @convention(c) (
    Int, DispatchQueue, @escaping (CFString) -> Void
) -> Void
typealias MRMediaRemoteRegisterForNowPlayingNotificationsFunction = @convention(c) (DispatchQueue)
    -> Void
typealias MRMediaRemoteSendCommandFunction = @convention(c) (UInt32, UnsafeMutableRawPointer?) ->
    Bool
...
let MRMediaRemoteGetNowPlayingApplicationDisplayName = unsafeBitCast(
    MRMediaRemoteGetNowPlayingApplicationDisplayNamePointer,
    to: MRMediaRemoteGetNowPlayingApplicationDisplayNameFunction.self)
...
let MRMediaRemoteSendCommand = unsafeBitCast(
    MRMediaRemoteSendCommandPointer, to: MRMediaRemoteSendCommandFunction.self)

// Add MediaRemote constants
let kMRMediaRemoteNowPlayingInfoDidChangeNotification = NSNotification.Name(
    "kMRMediaRemoteNowPlayingInfoDidChangeNotification")
let kMRMediaRemoteNowPlayingApplicationIsPlayingDidChangeNotification = NSNotification.Name(
    "kMRMediaRemoteNowPlayingApplicationIsPlayingDidChangeNotification")
let kMRMediaRemoteNowPlayingApplicationDidChangeNotification = NSNotification.Name(
...
let kMRPlaybackQueueContentItemsChangedNotification =
    "kMRPlaybackQueueContentItemsChangedNotification"
let kMRMediaRemoteNowPlayingApplicationClientStateDidChange =
...
func getUptime() -> String {
    ...
    if sysctl(&mib, 2, &boottime, &size, nil, 0) != -1 {
        ...
        if hours > 0 {
            return
                "up \(hours) \(hours == 1 ? "hour" : "hours"), \(minutes) \(minutes == 1 ? "minute" : "minutes")"
        } else {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        sketchyTask.arguments = [
            "sketchybar", "--query", currentPanel == .menu ? "apple.logo" : "date",
        ]
        ...
        if let barInfo = try? JSONSerialization.jsonObject(with: barData, options: [])
            as? [String: Any],
            ...
            displayRect["origin"] as? [Double] != nil
        {
            ...
            if let window = window {
                window.maxSize = NSSize(
                    width: windowWidth, height: (windowY / 1.7) - CGFloat(gapSize))
...
struct Card: View {
    ...
    init(
        content: AnyView, backgroundColor: Color, padding: EdgeInsets, backgroundImage: NSImage?,
        gradientOpacity: Double = 0.8
    ) {
...
struct ContentView: View {
    ...
    private func getUserProfileImage() -> NSImage? {
        ...
        if let imagePath = try? FileManager.default.contentsOfDirectory(
            atPath: "/Users/\(username)/Library/UserPictures"
        ).first {
        ...
        VStack(spacing: 2) {
            HStack(spacing: 0) {
                ...
                HStack(spacing: 8) {
                    ...
                    ActionButton(
                        icon: "rectangle.portrait.and.arrow.right", color: Color(Colors.blue)
                    ) {
                        logout()
                    }
                }
                ...
                ZStack {
                    ...
                    HStack(spacing: 12) {
                        ...
                        HStack(spacing: 20) {
                            ...
                            MediaControlButton(
                                systemName: mediaController.isPlaying ? "pause.fill" : "play.fill"
                            ) {
            ...
            VStack(spacing: 10) {
                ...
                LazyVGrid(
                    ...
                    CircularProgressView(
                        ...
                        icon: batteryIcon(
                            level: statsController.batteryLevel,
                            isCharging: statsController.isCharging),
                        color: Color(Colors.green)
                    )
                    CircularProgressView(
                        progress: statsController.volumeLevel,
                        icon: volumeIcon(
                            level: statsController.volumeLevel, isMuted: statsController.isMuted),
...
class StatsController: ObservableObject {
    ...
    private func updateMemoryUsage() {
        ...
        if result == KERN_SUCCESS {
            ...
            let total =
                Double(
                    stats.active_count + stats.inactive_count + stats.wire_count + stats.free_count)
                * Double(pageSize)
    ...
    private func updateCPUUsage() {
        ...
        var size = mach_msg_type_number_t(
            MemoryLayout.size(ofValue: hostInfo) / MemoryLayout<Int32>.size)
    ...
    private func updateBatteryStatus() {
            ...
            let sources = IOPSCopyPowerSourcesList(powerSource)?.takeRetainedValue() as? [CFTypeRef]
        {
            ...
            for source in sources {
                if let description = IOPSGetPowerSourceDescription(powerSource, source)?
                    .takeUnretainedValue() as? [String: Any]
                {
                    DispatchQueue.main.async {
                        self.batteryLevel =
                            (description[kIOPSCurrentCapacityKey] as? Double ?? 0) / 100.0
                        self.isCharging =
...
class WeatherController: ObservableObject {
    ...
    private func updateWeather() {
        ...
        let envPath = homeDirectory.appendingPathComponent(
            ".config/sketchybar/helpers/event_providers/.env"
        ).path
                ...
                DispatchQueue.main.async {
                    if let currentWeather = weatherData.list.first {
                        ...
                        self?.forecast =
                            ...
                            .map {
                                (
                                    day: $0.key, high: $0.value.max, low: $0.value.min,
