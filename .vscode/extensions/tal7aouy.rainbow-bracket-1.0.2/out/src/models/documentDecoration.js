...
class DocumentDecoration {
    ...
    expandBracketSelection(editor) {
        ...
        editor.selections.forEach((selection) => {
            ...
            if (this.scopeSelectionHistory.length !== 0) {
                selections =
                    this.scopeSelectionHistory[this.scopeSelectionHistory.length - 1];
            }
            else {
    ...
    updateScopeDecorations() {
        ...
        for (const scope of scopes) {
            ...
            const position = this.settings.scopeLineRelativePosition
                ? Math.min(scope.close.range.start.character, scope.open.range.start.character)
                : 0;
    ...
    setVerticalLineDecoration(scope, event, fallBackPosition, verticleLineRanges) {
        ...
        const normalRanges = verticleLineRanges
            .filter((e) => e.valid)
            .map((e) => e.range);
    ...
    updateLowestLineNumber(contentChanges) {
        for (const contentChange of contentChanges) {
            this.lineToUpdateWhenTimeoutEnds = Math.min(this.lineToUpdateWhenTimeoutEnds, contentChange.range.start.line);
        }
    }
    updateDecorations() {
        if (this.settings.isDisposed) {
