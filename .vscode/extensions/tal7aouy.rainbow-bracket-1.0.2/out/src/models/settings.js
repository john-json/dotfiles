...
const colorMode_1 = require("../interfaces/colorMode");
...
class Settings {
    constructor(languageID, gutterIcons, documentUri) {
        ...
        const configuration = vscode.workspace.getConfiguration('RainbowBrackets', documentUri);
        ...
        if (this.colorMode === colorMode_1.default.Consecutive) {
            ...
            if (typeof orphanColor !== 'string') {
                throw new Error('consecutivePairColors[' +
                    (consecutiveSettings.length - 1) +
                    '] is not a string');
            }
            const colors = consecutiveSettings[consecutiveSettings.length - 2];
            if (!Array.isArray(colors)) {
                throw new Error('consecutivePairColors[' +
                    (consecutiveSettings.length - 2) +
                    '] is not a string[]');
            }
            consecutiveSettings
                .slice(0, consecutiveSettings.length - 2)
                .forEach((brackets, index) => {
                if (typeof brackets === 'string' || Array.isArray(brackets)) {
                    if (brackets.length !== 2) {
                        throw new Error('consecutivePairColors[' +
                            index +
                            "] requires 2 element, e.g. ['(',')']");
                    }
            ...
            independentSettings.forEach((innerArray, index) => {
                ...
                if (typeof brackets !== 'string' && !Array.isArray(brackets)) {
                    throw new Error('independentSettings[' +
                        index +
                        '][0] is not a string or an array of strings');
                }
                if (brackets.length < 2) {
    ...
    createScopeLineDecorations(color, top = true, right = true, bottom = true, left = true, yOffset) {
        ...
        if (yOffset !== undefined && yOffset !== 0) {
            borderStyle +=
                '; transform: translateY(' + yOffset * 100 + '%); z-index: 1;';
        }
